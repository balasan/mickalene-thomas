             ...
             .query(prismic.Predicates.at("document.id", id)).submit(function(err, response) {
                 ...
                 response.results.forEach(function(item) {
                     ...
                     if (item.data["product-v.variation"]) {
                         var mainVar = {};
                        mainVar.image = item.data["product-v.image"].value.main.url;
                        obj.vars.push(mainVar);

                        item.data["product-v.variation"].value.forEach(function(vari) {
                            var varObj = {};
                            varObj.quantity = 1;
                            varObj.sku = vari.variationSku ? vari.variationSku.value : null;
                            varObj.image = vari.variationImage.value.views.small.url;
                            varObj.description = vari.vartiationDescription.value;
