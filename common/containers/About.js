...
export default class About extends Component {
  ...
  render () {
      ...
      var videoUrl = null;
      var videoEl = null;
      var mobile = self.mobilecheck();

      if (data) {
        if (data.header) headerEl = (<h1 id="about-header">{data.header}</h1>);
        if (data.body && !this.textEl) {
          textEl = [];
          data.body.forEach(function(textBlock, i){
            if (!textBlock.text.spans.length) {
              textEl.push(<p key={i}>{textBlock.text.text}</p>);
            } else {
              if (textBlock.text.spans[0].type == 'hyperlink') {
                var url = textBlock.text.spans[0].data.value.url;
                textEl.push(<p  key={i}><a href={url} style={{color: 'blue'}} target="_blank">{textBlock.text.text}</a></p>);
              } else {
                textEl.push(<p key={i}>{textBlock.text.text}</p>);
              }
            }
